...
$(function(){
  ...
  function listTipoCaravana() {
          opcion = "tipocaravana";
          $.fn.dataTable.ext.errMode = 'throw';
          tablaTipoCaravana = $('#tabTipoCaravana').DataTable({  
            "paging":   true,
            "ordering": true,
            "info":     false,
            "bFilter": true,
  ...
  $('#frmAddTipoCaravana').submit(function(e){  
    ope= document.getElementById("txtOPE").value;     
    e.preventDefault();                      
    $.ajax({
        url: 'controller/manTipoCaravana.php',
        type: 'POST',
        data: $('#frmAddTipoCaravana').serialize(),
        success: function(res){
          tablaTipoCaravana.ajax.reload(null, false);
            $("#frmAddTipoCaravana").trigger("reset");
          if (ope==='GUA') {
    ...
    $(document).on("click", ".btnBorrar", function(e){
          e.preventDefault();    
          fila = $(this).closest("tr");	        
          idtip = parseInt(fila.find('td:eq(0)').text()); //capturo el ID		
          opera="ELI";
          $.ajax({
        url: 'controller/manTipoCaravana.php',
        type: 'POST',
        data: {txtidtip:idtip,txtOPE:opera},
        success: function(res){
          tablaTipoCaravana.ajax.reload(null, false);
